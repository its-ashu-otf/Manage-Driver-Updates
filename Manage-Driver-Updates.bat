@echo off
title Manage Driver Updates v7.0 by @its-ashu-otf
...
echo 2) Disable Driver Updates Offered By Windows Aggressive (NOT Recommended)
echo 3) Enable Driver Updates Offered By Windows
echo 4) Get current status info
echo ==========================================================
set /p op=Type option:
if "%op%"=="1" goto op1
if "%op%"=="2" goto op2
if "%op%"=="3" goto op3
if "%op%"=="4" goto details
echo Please pick a valid option.
goto begin

:details
echo.
echo ================== DRIVER POLICY STATUS DETAILS ==================
REM Check ExcludeWUDriversInQualityUpdate
set "drvStatus=Not Set"
for /f "tokens=3" %%A in ('reg query "HKLM\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" /v ExcludeWUDriversInQualityUpdate 2^>nul ^| find "REG_DWORD"') do (
    if "%%A"=="0x1" set "drvStatus=Disabled"
    if "%%A"=="0x0" set "drvStatus=Enabled"
)
REM Check PreventDeviceMetadataFromNetwork
set "metaStatus=Not Set"
for /f "tokens=3" %%A in ('reg query "HKLM\SOFTWARE\Policies\Microsoft\Windows\Device Metadata" /v PreventDeviceMetadataFromNetwork 2^>nul ^| find "REG_DWORD"') do (
    if "%%A"=="0x1" set "metaStatus=Blocked"
    if "%%A"=="0x0" set "metaStatus=Allowed"
)
echo   ExcludeWUDriversInQualityUpdate : %drvStatus%
echo     - Disabled = Driver updates are blocked by policy.
echo     - Enabled  = Driver updates are allowed.
echo     - Not Set  = Default Windows behavior.
echo.
echo   PreventDeviceMetadataFromNetwork: %metaStatus%
echo     - Blocked = Device metadata download is blocked.
echo     - Allowed = Device metadata download is allowed.
echo     - Not Set = Default Windows behavior.
echo ----------------------------------------------------------
if "%drvStatus%"=="Disabled" (
    if "%metaStatus%"=="Blocked" (
        echo   Current: [2] Aggressive - Drivers & metadata blocked
    ) else (
        echo   Current: [1] Disabled  - Drivers blocked by policy
    )
) else if "%drvStatus%"=="Enabled" (
    echo   Current: [3] Enabled   - Drivers allowed
) else (
    echo   Current: [0] Not Set   - Default Windows behavior
)
echo ==========================================================
echo.
pause
...
:: Delete the registry value "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\DriverSearching!SearchOrderConfig"
PowerShell -ExecutionPolicy Unrestricted -Command "reg delete 'HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\DriverSearching' /v 'SearchOrderConfig' /f 2>$null"
